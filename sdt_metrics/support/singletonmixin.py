...
# Authored by Gary Robinson and placed in the public domian
# Made Python 3k / nose compatible by Roger Lew 06.10.12
...
# Ensure only one instance of each Singleton
...
def forgetAllSingletons():
    '''
    This is useful in tests, since it is hard to know
    which singletons need to be cleared to make a test work.'''
    _lockForSingletons.acquire()
    try:
...
class Singleton(object):
    ...
    def getInstance(cls, *lstArgs, **dctKwArgs):
        ...
        if cls._isInstantiated():
            if (lstArgs or dctKwArgs) and not hasattr(cls, 'ignoreSubsequent'):
                raise SingletonException('Singleton already instantiated, '
                                         'but getInstance() called with args.')
        else:
    ...
    def _isInstantiated(cls):
        # Don't use hasattr(cls, 'cInstance'), because
        # that screws things up if there is a singleton that
        # extends another singleton.  hasattr looks in the
        # base class if it doesn't find in subclass.
    ...
    def _forgetClassInstanceReferenceForTesting(cls):
        ...
        try:
            ...
            # run up the chain of base classes until we find
            # the one that has the instance and then delete it there
    ...
    _forgetClassInstanceReferenceForTesting = \
            classmethod(_forgetClassInstanceReferenceForTesting)
